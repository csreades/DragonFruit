...
- Visualizes the current cross-section as a filled white cap at the slice height.
...
- Cross-section cap (filled surface)
  - `CrossSectionCap` intersects mesh triangles with plane Z = current slice height.
...
- Files of note
  ...
  - `src/components/scene/SceneCanvas.tsx`: R3F Canvas with Z-up camera, lighting, helpers, Z-axis clipping planes, integration of the STL mesh and cross-section cap.
  - `src/components/scene/CrossSectionCap.tsx`: Cross-section polygon extraction at Z heights, creates horizontal XY plane shapes with proper positioning and depth testing.
  ...
  - `src/app/page.tsx`: UI composition, file input, layer height input, layer index state, mapping from index to Z-height, scene wiring, and model info overlay.
...
- Added cross-section cap computation at Z heights with proper XY plane positioning and depth testing.
